...
/// <summary>
/// Continuously watches Apple DFU/Pongo on the Windows host and repairs only when
/// the device leaves a host-owned WinUSB/libusb backend.
/// </summary>
public sealed class LibusbKWatchdog : IDisposable
{
    ...
    private readonly Dictionary<ushort, int> _badServiceStreak = new();
    ...
    public void Start()
    {
        ...
        lock (_startStopLock)
        {
            ...
            Emit("driver-watch", "USB watchdog started (DFU/Pongo accepted on WINUSB or libusbK).");
    ...
    public void Stop()
    {
        ...
        Emit("driver-watch", $"USB watchdog stopped (repairs this session: {ReinstallCount}).");
    }
    ...
    private async Task TickAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var device in devices)
        {
            ...
            if (DriverInstaller.IsHostUsbBackendService(service))
            {
                _badServiceStreak[device.ProductId] = 0;
                continue;
            }
            ...
            if (string.IsNullOrWhiteSpace(service))
            {
                Emit(
                    "driver-watch",
                    $"PID 0x{device.ProductId:X4} service is not settled yet; waiting before any repair.");
                continue;
            }
            ...
            _badServiceStreak.TryGetValue(device.ProductId, out var streak);
            streak++;
            _badServiceStreak[device.ProductId] = streak;
            if (streak < 2)
            {
                Emit(
                    "driver-watch",
                    $"PID 0x{device.ProductId:X4} saw service '{service}' once; waiting for a stable second read before repair.");
                continue;
            }
            ...
            if (!_settings.AutoInstallDrivers)
            {
                if (!_loggedAutoOff)
                {
                    ...
                    Emit(
                        "driver-watch",
                        $"PID 0x{device.ProductId:X4} needs WINUSB/libusbK but AutoInstallDrivers is off (now={service ?? "none"}).",
                        isError: true);
                }
            ...
            if (!Elevation.IsAdmin())
            {
                if (!_loggedNotAdmin)
                {
                    ...
                    Emit(
                        "driver-watch",
                        "Cannot auto-repair the DFU USB backend — app is not elevated. Relaunch as Administrator.",
                        isError: true);
                }
            ...
            // Retry wdi-simple up to 3 times within this tick. Stop as soon as Windows
            // reports either WINUSB or libusbK; both are usable by the bundled libusb tools.
            ...
            for (var retry = 0; retry < maxRetries; retry++)
            {
                ...
                Emit(
                    "driver-watch",
                    $"Repairing USB backend via wdi-simple for PID 0x{device.ProductId:X4} " +
                ...
                Emit(
                    ...
                    isError: !ok && !DriverInstaller.IsHostUsbBackendService(after));

                if (DriverInstaller.IsHostUsbBackendService(after))
                {
                    break;
                }

                // Not yet on a host-owned backend — retry immediately if we have budget.
                if (retry < maxRetries - 1)
                {
                    Emit("driver-watch", $"Still not WINUSB/libusbK (={after ?? "none"}) — retrying immediately...");
