...
public sealed class AppleUsbMonitor : IDisposable
{
    ...
    private void MergeUsbipdBusIds(List<AppleUsbDevice> devices)
    {
        try
        {
            ...
            foreach (var usbipdDevice in UsbipdService.ParseAppleDevices(listOutput))
            {
                var parts = usbipdDevice.VidPid.Split(':', StringSplitOptions.TrimEntries);
                if (parts.Length != 2 ||
                    !ushort.TryParse(parts[1], System.Globalization.NumberStyles.HexNumber, null, out var productId))
                {
                    continue;
                }

                if (devices.Any(d => d.ProductId == productId && string.Equals(d.BusId, usbipdDevice.BusId, StringComparison.OrdinalIgnoreCase)))
                {
                    continue;
                }

                var syntheticDeviceId = $"USB\\VID_05AC&PID_{productId:X4}\\USBIPD_{usbipdDevice.BusId}";
                devices.Add(AppleUsbDevice.FromPnpEntity(
                    syntheticDeviceId,
                    ExtractUsbipdName(usbipdDevice.RawLine, usbipdDevice.VidPid),
                    "Present",
                    usbipdDevice.State == UsbipdAttachState.Attached ? "usbipd" : string.Empty,
                    usbipdDevice.BusId));
            }
        }
        catch
    ...
    private static string ExtractUsbipdName(string rawLine, string vidPid)
    {
        var vidPidIndex = rawLine.IndexOf(vidPid, StringComparison.OrdinalIgnoreCase);
        if (vidPidIndex < 0)
        {
            return "Apple USB device";
        }

        var afterVidPid = rawLine[(vidPidIndex + vidPid.Length)..].Trim();
        foreach (var state in new[] { "Not shared", "Attached", "Shared", "Error" })
        {
            var stateIndex = afterVidPid.LastIndexOf(state, StringComparison.OrdinalIgnoreCase);
            if (stateIndex >= 0)
            {
                afterVidPid = afterVidPid[..stateIndex].Trim();
                break;
            }
        }

        return string.IsNullOrWhiteSpace(afterVidPid) ? "Apple USB device" : afterVidPid;
    }
