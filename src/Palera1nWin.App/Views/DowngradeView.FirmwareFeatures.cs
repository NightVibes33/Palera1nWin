...
public partial class DowngradeView
{
    ...
    private string? DetectedProductType => _firmwareIdentifier ?? ResolveProductTypeFromInspection();

    private string? ResolveProductTypeFromInspection()
    {
        var fromCurrent = NormalizeProductType(_experienceSnapshot.ProductType) ??
                          NormalizeProductType(_monitor.Current.ProductType) ??
                          NormalizeProductType(_activeDeviceProfile?.ProductType);
        if (fromCurrent is not null &&
            _inspection?.MatchesProductType(fromCurrent) == true &&
            DarkSwordDeviceCatalog.IsSupported(fromCurrent))
        {
            return fromCurrent;
        }
        ...
        var supported = _inspection.SupportedProductTypes
            .Where(DarkSwordDeviceCatalog.IsSupported)
        ...
        if (supported.Contains("iPad6,11", StringComparer.Ordinal) &&
            supported.Contains("iPad6,12", StringComparer.Ordinal) &&
            (_experienceSnapshot.InstanceId?.Contains("001858A43038A0A6", StringComparison.OrdinalIgnoreCase) == true ||
             _monitor.Current.InstanceId?.Contains("001858A43038A0A6", StringComparison.OrdinalIgnoreCase) == true))
        {
            return "iPad6,11";
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var irecovery = ResolveToolchainExecutable("irecovery.exe");
    ...
    private string ResolveToolchainExecutable(string fileName)
    {
        var nativePath = Path.Combine(_tools.Root, "native", fileName);
        return File.Exists(nativePath) ? nativePath : Path.Combine(_tools.Root, fileName);
    }
