...
public partial class DowngradeView
{
    ...
    private void UpdateExperienceDeviceState(AppleDeviceSnapshot snapshot)
    {
        ...
        {
            InvalidatePreflight("Device mode, driver, identity, or firmware changed. Run preflight again.");
        }
        UpdateActionState();
    }
    ...
    private async Task<bool> RunPreflightAsync(bool showResultDialog)
    {
        ...
        if (!File.Exists(IpswPathBox.Text) || string.IsNullOrWhiteSpace(DetectedProductType))
        {
            ShowMessage(
                "Detect the exact device and select an inspected iOS/iPadOS 15 IPSW first.",
                "Preflight not ready",
    ...
    private bool IsEnhancedSafetyReady()
    {
        var backupReady = BackupEncryptedCheck.IsChecked == true &&
    ...
    private async Task ResumeLatestSessionAsync()
    {
        ...
        InvalidatePreflight("Recovery session loaded. Run preflight for the current device and driver state.");
        if (!await RunPreflightAsync(showResultDialog: false)) return;
